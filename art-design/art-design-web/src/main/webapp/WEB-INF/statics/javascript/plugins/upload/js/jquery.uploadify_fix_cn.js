...
(function($) {
	...
	var handlers = {
		...
		onDialogOpen : function() {
			...
			this.queueData.errorMsg       = '文件没有加入到队列中:';
		...
		onSelect : function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					var replaceQueueItem = confirm('文件 "' + file.name + '" 在队列中已经存在.\n是否替换?');
					if (!replaceQueueItem) {
				...
				switch(errorCode) {
					case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
						if (settings.queueSizeLimit > errorMsg) {
							//this.queueData.errorMsg += '\n上传文件的数量超过限制 (' + errorMsg + ').';
							this.queueData.errorMsg += '\n上传文件的数量超过限制 .';
						} else {
							this.queueData.errorMsg += '\n上传文件总大小超过限制(' + settings.queueSizeLimit + ').';
						}
						break;
					...
					case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
						this.queueData.errorMsg += '\n不支持文件 "' + file.name + '" 的类型 (' + settings.fileTypeDesc + ').';
						break;
				}
			}
		...
		onUploadError : function(file, errorCode, errorMsg) {
			...
			switch(errorCode) {
				...
				case SWFUpload.UPLOAD_ERROR.MISSING_UPLOAD_URL:
					errorString = '没有设置 upload_url';
					break;
				case SWFUpload.UPLOAD_ERROR.IO_ERROR:
					errorString = '服务器 I/O 错误';
					break;
				case SWFUpload.UPLOAD_ERROR.SECURITY_ERROR:
					errorString = '服务器安全验证错误';
					break;
				case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
//					alert('上传的文件超过了允许的最大值 (' + errorMsg + ').');
					errorString = '上传的文件超过了允许的最大值';
					break;
			...
			// Call the default event handler
			if ($.inArray('onUploadError', settings.overrideEvents) < 0 && file && file.id) {
		...
		onUploadStart : function(file) {
			...
			if (settings.checkExisting) {
				$.ajax({
					...
					success : function(data) {
						if (data == 1) {
							var overwrite = confirm('文件 "' + file.name + '" 已经存在.\n是否替换?');
							if (!overwrite) {
								this.cancelUpload(file.id);
								$('#' + file.id).remove();
			...
			if (settings.onUploadStart) {
				//settings.onUploadStart.bind(this);
