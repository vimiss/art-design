				...
				<div class="col-xs-12">
					<form action="${ctx}/periodApply/listConfirmApply" method="POST" id="queryForm_ajax">
								...
								<div class="col-xs-12" style="margin-top:10px;">
									...
									<span onclick="goEffect();" class="BtnSouSu default-btn btn-blue">全部生效</span>
									<span onclick="goSelectedEffect();" class="BtnSouSu default-btn btn-blue">生效</span>
									<!-- <span onclick="goUnEffectApply();" class="BtnSouSu default-btn btn-blue">失效</span> -->
									<!-- 	&nbsp;&nbsp;&nbsp;注:当分期额度不够时，建议按照打款时间的先后顺序，先打款的先确认生效。 -->
								</div>
						<div class="col-xs-12 ">
									<table class="table table-striped table-bordered table-hover RtableStriped RtableStropSOS">
										...
										<#else>
											<tr align="center"><td colspan="15"><font color="red" size="4">该分期下订单已经全部生效！</font></td></tr>
										</#if>
										</tbody>
									</table>
...
<script>
	$(function(){
		...
		$("#queryCommit_ajax").click(function(){
			...
			$("#pageNow_ajax").val(1);
			$("#periodEffectModal").modal("hide");
			$("#periodEffectModal").on("hidden.bs.modal", function() {
				queryEffect($("#queryForm_ajax").serialize());
