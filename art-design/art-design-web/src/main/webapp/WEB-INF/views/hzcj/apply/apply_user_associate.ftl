	...
	<div class="modal-content">
		...
		<div class="modal-footer">
			<div class="text-center">
				<a href="javascript:;" onclick="submitUserAdvisor(${relationListByUserId.userId},${relationListByUserId.relaUserId});" class="default-btn btn-orange noBorder marginLR20">确 定</a>
...
<script>
	function submitUserAdvisor(userId,relaUserId) {
		var applyUserName = "${applyUserName}";
		var applyAdvisorName = "${applyAdvisorName}";
		Modal.confirm({msg:"确认这个用户和此投资顾问吗？", okCallback:function() {
			$(this).on("hidden.bs.modal", function() {
				$("#user_associate_modal").modal("hide");
				$("#user_associate_modal").on("hidden.bs.modal", function() {
					$("#applyAdvisorId").val(relaUserId);
					$("#applyUserId").val(userId);
					$("#applyAdvisorName").val(applyAdvisorName);
					$("#applyUserName").val(applyUserName);
