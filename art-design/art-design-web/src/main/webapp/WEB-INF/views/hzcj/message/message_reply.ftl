...
<div class="modal-dialog modal-lg1" role="document" style="width:1400px;">
		...
		<form id="msgpushForm" name="msgpushForm" method="post" >
			<!-- 文本内容开始 -->
							 <div class="col-xs-12 MylistCol">
								...
								<div class="pull-left MylistRight">
									<select class="chosen-select AceSelect" name="messType" id="messType">
											<@option name="message_type_param" status="1" />
										</select>
	                      	...
	                      	<div class="col-xs-12 MylistCol">
								...
								<div class="pull-left MylistRight">
									<input type="text" name="messTitle" id="messTitle" class="InputName1" maxlength="50"  placeholder="限50字符(中文字符长度为2)" >
							 ...
							 <div class="col-xs-12 MylistCol">
								...
								<div class="pull-left MylistRight">
										<script id="content"  type="text/plain" style="margin-right:15px;height:700px;">
						 				</script>
								</div>
							</div>
								<input type="hidden" name="messContent" id="messContent">
	                      	<!-- 文本内容结束 -->
	                      	<input type="hidden" name="addMessage" value="${addMessage?if_exists}" />
		</form>
		</div>
	...
	function checkPersonalCenterForm(){
		...
		if(!$("#messType").val()){
			$("#checkMessType").html("<font>请选择消息类型!</font>");
			return;
		}else{
			$("#checkMessType").empty();
		}
		$("#messContent").val(UE.getEditor('content').getContent());
		$.ajax({
			type : "POST",
			url : ctx + "/message/token/add",
			...
			success:function(data) {
				if(data.result == 'success'){
					Modal.alert({msg:"新增消息成功",okCallback:function(){
					...
					$("input[name='addMessage']").val(data.newTokenName);
