...
<#include "commons/head.ftl">
                    	...
                    	<form id="msgpushForm" name="msgpushForm" method="post">
	                      	...
	                      	<div class="col-xs-12 MylistCol">
								...
								<div class="pull-left MylistRight">
									<input type="hidden" id="fenceId" name="fenceId" value="${itemFence.fenceId}">
										<input type="text" id="fenceName" name="fenceName" value="${itemFence.fenceName}" class="InputName0"  maxlength="30"  placeholder="限30字符(中文字符长度为2)">
							...
							<div class="col-xs-12 MylistCol">
								...
								<div class="pull-left MylistRight">
										<input type="text" id="fenceTableCode" name="fenceTableCode" value="${itemFence.fenceTableCode}" class="InputName0"  maxlength="30"  placeholder="限30字符(中文字符长度为2)" disabled="disabled">
							...
							<div class="col-xs-12 MylistCol">
								...
								<div class="pull-left MylistRight">
										<select class="chosen-select AceSelect" id="fenceFlag" name="fenceFlag" >
											<@option name="yes_no_type_param" status="1" value="${itemFence.fenceFlag}"/>
										</select>
										<span id="checkFenceFlag" class="red" style="margin-left: 20px;"></span>
								</div>
							</div>
							<input type="hidden" name="modifyItemFence" value="${modifyItemFence?if_exists}">
	                      	<!-- 文本内容结束 -->
	                  </form>
	...
	function checkPersonalCenterForm(){
		...
		var fenceFlag= $("#fenceFlag").val();
		if(!fenceFlag||fenceFlag==''){
	...
	function modify(){
		$.ajax({
			type : "POST",
			url : ctx + "/itemFence/token/modify",
			data : $("#msgpushForm").serialize(),
			async : false,
			dataType:"json",
			success:function(result) {
				if(result.code == "0"){
					Modal.alert({msg:"栏目修改成功",okCallback:function(){
						window.location.href=ctx+'/itemFence/list';
					}
				});
				}else{
					$("input[name='modifyItemFence']").val(result.value.newTokenName);
