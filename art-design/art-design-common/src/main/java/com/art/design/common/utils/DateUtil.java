package com.art.design.common.utils;
...
//        System.out.println(DateUtil.format2AddLastTime(new Timestamp(d.getTime())));
	...
	public static int getTodayTime() throws ParseException {
		Calendar cal = Calendar.getInstance();
		String today = new SimpleDateFormat("yyyy-MM-dd ").format(cal.getTime());
		return DateUtil.stringDateToInt(today);
	}
     ...
     * @return
     */
    public static String intToDate2(int date, String format) {
    ...
    public static Integer getZhiDingDayDateAfter(Integer cdate, Integer day) throws Exception{
        if (cdate == null || cdate < 0) {
            cdate = getCurrentTime();
        }

        cdate = cdate+(day* Constant.SECONDS_OF_ONE_DAY);

        int x = getMonthByCdate(cdate);
    ...
    public static Date dateIntToDate(Integer time) {
        if(time != null && time.intValue() != 0) {
            ...
            try {
                Long e = new Long(time + "");
                String d = format.format(Long.valueOf(e.longValue() * 1000L));
                date = format.parse(d);
